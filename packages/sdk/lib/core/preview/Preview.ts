...
export default class Preview extends THREE.EventDispatcher<PreviewerEventMap> {
	 ...
	 * 加载默认环境和背景
	 */
	loadEnv(options?: { setBg?: boolean; extension?: string; onLoad?: (texture: THREE.Texture) => void; onError?: (error: Error) => void }) {
	...
	load(fileOrUrl: string | File, type: string = "Model") {
		return new Promise(async (resolve, reject) => {
			...
			switch (type) {
				case "Model":
					Loader.loadFile(file, new THREE.LoadingManager(), null, false)
						.then(model => {
