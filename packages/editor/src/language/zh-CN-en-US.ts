export default {
	...
	layout: {
		...
		scene: {
			...
			plane: {
				...
				"Sub Color": "次颜色",
				Size: "尺寸",
				"Sub Color Distance": "次颜色显隐藏距离",
				Current: "当前",
				Axes: "坐标轴",
				Color: "颜色",
				"Preset Colors": "预置颜色",
				"Preset Textures": "预置贴图",
				"Texture Image": "贴图图片",
				"Custom Image URL": "自定义图片地址",
				None: "无",
				Opacity: "透明度",
				Repeat: "平铺次数",
				Rotation: "贴图旋转",
			...
			tools: {
				...
				Start: "起点",
				End: "终点",
				"No Path Points": "暂无路径点，请在场景中点击拾取",
				"Path Points": "路径点",
				"Path Length": "路径长度",
				"Flow Speed": "流动速度",
				"Click to draw path": "点击加点，双击或点「完成」生成路径",
				"Path generated": "路径已生成，箭头指示前进方向",
			},
		},
		sider: {
			Scene: "场景",
			...
			terrain: {
				...
				"Preset Tilesets": "选择预置数据集",
				"Preset Tileset Hint": "选择后会将地形原点移到数据所在位置、平移影像范围，并重置模型的偏移/旋转/缩放。",
				"Dayanta Photogrammetry": "大雁塔（西安 · 倾斜摄影）",
				"Lujiazui Buildings": "陆家嘴建筑群（上海 · 城市白模）",
				"Cesium LOD Sample": "Cesium 官方 LOD 示例（费城）",
				"Preview Unavailable": "预览不可用",
				"OSM Humanitarian": "OSM 人道主义风格",
				"OSM France": "OSM 法国风格",
				CyclOSM: "CyclOSM 骑行图",
				"Esri Imagery": "Esri 全球影像",
				"Esri Street": "Esri 街道图",
				"Esri Topo": "Esri 地形图",
				"Esri Shaded Relief": "Esri 晕渲地貌",
				"Esri NatGeo": "Esri 国家地理",
				"Esri Light Gray": "Esri 浅灰底图",
				"Esri Dark Gray": "Esri 深灰底图",
				...
				SSE: "屏幕空间误差",
				"Terrain Outline": "地形描边",
				"Outline Color": "描边颜色",
				"Outline Width": "描边宽度",
				"Sync Origin": "同步地形原点",
				Sync: "同步",
				Placement: "定位",
				"Anchor Unavailable": "瓦片集未加载或不含地理参考，无法获取位置",
				"Origin Synced": "地形原点已与 3D Tiles 对齐",
				"Edit In Viewport": "选中编辑",
				"Update Anchor": "更新锚点",
				Update: "更新",
				"Anchor Updated": "已将当前偏移合并到定位坐标",
				"Reset Transform": "变换复位",
				Reset: "复位",
				"Transform Reset": "已复位为瓦片集自带位置与姿态",
				"Snap To Ground": "高度贴地",
				Snap: "贴地",
				"Ground Snapped": "模型地面已与影像平面贴合",
				"Load State": "加载状态",
				Loading: "加载中",
				Loaded: "已加载",
				"Load Error": "加载失败",
	...
	plugin: {
		...
		shaderPlayground: {
			"Shader playground": "着色器沙盒",
			Reset: "重置",
			Close: "关闭",
			"Next topic": "下一个知识点",
			"No topics in this grade": "该难度暂无知识点",
			"Preview geometry": "预览几何",
			Uniforms: "参数",
			Vertex: "顶点着色器",
			Fragment: "片元着色器",
			"Free mode hint": "自由模式：随意改代码看预览。可从难度模式带入当前知识点再改。",
			"Lesson mode": "难度模式",
			"Free mode": "自由模式",
			Beginner: "入门",
			Intermediate: "进阶",
			...
			"Solid color": "纯色片元",
			"Solid color summary":
				"片元着色器会对「每一个像素」各跑一遍 main。这里整屏同一颜色——你改的是公式，不是在「选中某个物体上色」。",
			"Solid color hint": "试试改 uColor，或把 uIntensity 调到 0.3，观察整屏变暗。",
			"Meet UV": "认识 UV",
			"Meet UV summary": "vUv 是几何自带的 0～1 坐标（左下接近 0,0）。用 vUv.x / vUv.y 当颜色，就能「看见」UV 网格。",
			"Meet UV hint": "把 vec3(vUv.x, vUv.y, 0.4) 改成 vec3(vUv.x) 只看横向渐变。",
			"Screen vs UV": "屏幕坐标 vs UV",
			"Screen vs UV summary":
				"左边用 gl_FragCoord/分辨率（屏幕像素），右边用 vUv。闪电的天空闪照更像左边；丝带电弧横截面更像右边。",
			"Screen vs UV hint": "拖动窗口大小时看左边图案是否跟着变（分辨率相关）。",
			"Distance to shape": "距离变成形状",
			"Distance to shape summary":
				"圆形不是贴图识别出来的：每个像素算到中心的距离，够亮才画，太暗就 discard——空白区=公式结果太弱。",
			"Distance to shape hint": "把 uRadius 从 0.22 改到 0.4，圆变大；体会「阈值」如何变成可见边界。",
			"Time pulse": "时间与闪烁",
			"Time pulse summary":
				"uTime 由预览循环每帧更新。用 sin(uTime * uSpeed) 就能做呼吸灯——闪电的回击强度也是时间驱动的。",
			"Time pulse hint": "调大 uSpeed，闪得更快。",
			"Additive blob": "叠加混合亮斑",
			"Additive blob summary":
				"Additive 把光叠到背景上，只加不挡。闪电光晕常用这种混合；能量写进 RGB、alpha 固定 1 更稳。",
			"Additive blob hint": "本关已开 additive。把强度调高，看亮斑如何叠亮背景。",
			"Bolt profile": "闪电丝带剖面",
			"Bolt profile summary":
				"用 vUv.x 模拟「离电弧中线的距离」，多层 exp(-k d²) 做出白芯+蓝晕。这与工程里 Lighting 电弧片元同一思路。",
			"Bolt profile hint": "分别调大芯锐度 / 减小光晕锐度。读完可看文档「着色器进阶-闪电案例」。",
			"Open document": "新标签预览文档",
			"No document": "当前知识点暂无文档",
			"Enter fullscreen": "全屏",
			"Exit fullscreen": "退出全屏",
			"Editor fullscreen": "编辑器全屏",
			"Exit editor fullscreen": "退出编辑器全屏",
		},
	},
	extra: {
		"New animation": "新增动画",
