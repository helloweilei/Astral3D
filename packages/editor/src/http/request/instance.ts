...
import { handleAxiosError, handleBackendError, handleResponseError, handleServiceResult } from "@/utils/service";
		...
		this.instance.interceptors.response.use(
			async response => {
				...
				if (status === 200 || status < 300 || status === 304) {
					...
					// 判断返回文件流的情况
					if ((response.headers["content-type"] as string)?.includes("application/octet-stream")) {
